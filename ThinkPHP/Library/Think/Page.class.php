...
class Page{
    ...
    public function show() {
        ...
        $up_page = '<a class="prev btn';
        if($this->nowPage == 1){
            $up_page .= ' disabled';
        }
        $up_page .= '" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a>';
        
        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = '<a class="next btn';
        if($this->nowPage == $this->totalPages){
            $down_page .= ' disabled';
        }
        $down_page .= '" href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a>';

        //第一页
        $the_first = '<a class="first btn';
        if($this->nowPage == 1){
            $the_first .= ' disabled';
        }
        $the_first .= '" href="' . $this->url(1) . '">' . $this->config['first'] . '</a>';

        //最后一页
        $the_end = '<a class="end btn';
        if($this->nowPage == $this->totalPages){
            $the_end .= ' disabled';
        }
        $the_end .= '" href="' . $this->url($this->totalPages) . '">' . $this->config['last'] . '</a>';
